...
async function findDirectories(startPath, relativePath = '', rootDirectory = '', level = 0) {
    let result = {};
    ...
    for (let file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            directories.push({ isDir: true, title: file, name: file, path: filename, relativePath: relativeFilename });
        } else {
            if (filename.endsWith('.md')) {
                let frontMatter = matter.read(filename);
                let metadata = frontMatter.data;
                let title = metadata.title || path.basename(filename, '.md');
                let weight = (frontMatter.data.weight !== null && frontMatter.data.weight !== undefined) ? frontMatter.data.weight : Infinity; // Default weight for files without weight
                ...
                let toolbar_title = metadata.toolbar_title || path.basename(filename, '.md'); //Used on the navbar
                fileObjs.push({ id, title, toolbar_title, filePath: relativeFilename.replace(/\\/g, '/'), file, date, weight, description_title, description, image, level: offseted_level });
            }
        }
    }
    ...
    result.sortedOrder = [...fileObjs, ...directories].sort((a, b) => {
    ...
    for (let dir of directories) {
        result[dir.name] = await findDirectories(dir.path, dir.relativePath, !rootDirectory ? dir.name : rootDirectory, level + 1);
    }
...
async function addWeightToDirectories(directories) {
    for (let dir of directories) {
        ...
        try {
            ...
            dir.weight = (frontMatter.data.weight !== null && frontMatter.data.weight !== undefined) ? frontMatter.data.weight : Infinity; // Default weight for files without weight
        } catch (err) {
...
(async () => {
    const directories = await findDirectories('./projects/example-app/src/assets/categories', 'assets/categories');
    await fs.writeFile('./projects/example-app/src/assets/categories/categories.json', JSON.stringify(directories));
