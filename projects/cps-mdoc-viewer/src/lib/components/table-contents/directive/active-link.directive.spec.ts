...
describe('ActiveLinkDirective', () => {
  globalThis.CSS = { escape: jest.fn().mockImplementation(val => val) } as any;
  ...
  beforeEach(() => {
    ...
    directive = fixture.debugElement.query(By.directive(ActiveLinkDirective)).injector.get(ActiveLinkDirective);
  });
  ...
  it('should properly set up click event handlers', fakeAsync(() => {
    ...
    tick();
    fixture.detectChanges();
    const section1Link: HTMLAnchorElement = fixture.nativeElement.querySelector('a[href="#section1"]');
    section1Link.click();
    expect(spy).toHaveBeenCalled();
    expect(globalThis.window.scrollTo).toHaveBeenCalledWith({ "behavior": "smooth", "top": 0 });
  }))
});
